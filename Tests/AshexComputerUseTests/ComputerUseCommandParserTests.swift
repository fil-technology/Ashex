...
@Test func parsesDeterministicPrototypeCommands() throws {
    ...
    #expect(try parser.parse("press space") == .pressKey(key: "Space", modifiers: []))
    #expect(try parser.parse("press delete") == .pressKey(key: "Delete", modifiers: []))
    #expect(try parser.parse("press left") == .pressKey(key: "Left", modifiers: []))
    ...
    #expect(try parser.parse("scroll up 3") == .scroll(direction: .up, amount: 3))
