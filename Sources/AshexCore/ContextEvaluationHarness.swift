...
public struct ContextEvaluationHarness: Sendable {
    ...
    public func evaluate(
        ...
        let queries = cases.map { item -> ContextEvaluationQueryResult in
            ...
            let firstRelevantRank = returnedFiles.firstIndex { item.expectedFiles.contains($0) }
                .map { $0 + 1 }
        ...
        let meanReciprocalRank = cases.isEmpty ? 0 : reciprocalRanks.reduce(0, +) / Double(cases.count)
